...
def seed_db_command():
    ...
    # --- 2. BUAT DATA USER ---
    # Felix: Admin, Punya UMKM Lengkap, Verified
    ...
    user_felix.set_password('password')

    # Dewa: Panitia, Verified
    ...
    user_dewa.set_password('password')

    # Bayu: User Biasa, Tidak Punya UMKM, Verified
    ...
    user_bayu.set_password('password')
    
    # Rayyan: User Belum Terverifikasi (Demo Register)
    ...
    user_rayyan.set_password('password')

    # Citra: User UMKM, Verified (Demo CRUD UMKM)
    ...
    user_citra.set_password('password')
    ...
    # Event 1: Offline, Wajib UMKM, Ada Pre/Post Test
    event_offline_wajib = Event(
        ...
        has_pre_post_test=True, # ADA TEST
        online_event_url=None
    )
    
    # Event 2: Online, Umum (Tanpa Test)
    event_online_umum = Event(
        id=2, sifat_pelatihan='Wajib', title='[ONLINE] Dasar Digital Marketing', 
        jenis_event='Private', tempat_event='Online', # <-- CUMA ONLINE
    ...
    # Event 3: Online, Besok (Untuk Demo Notif H-1)
    event_reminder_test = Event(
        id=3, sifat_pelatihan='Umum', title='[DEMO] Event Besok (Test Notif)', 
        jenis_event='Public', tempat_event='Online', # <-- CUMA ONLINE
    ...
    # Event 4: Selesai (Untuk Demo Laporan)
    event_selesai = Event(
    ...
    # --- 5. BUAT TIKET DEMO ---
    t1 = Ticket(user_id=1, event_id=1) # Felix @ Event Wajib (Test)
    t2 = Ticket(user_id=3, event_id=2) # Bayu @ Event Online
    t3 = Ticket(user_id=5, event_id=3) # Citra @ Event H-1
    t4 = Ticket(user_id=3, event_id=4) # Bayu @ Event Selesai
    db.session.add_all([t1, t2, t3, t4])
    db.session.commit()

    # --- 6. BUAT CHECK-IN DEMO ---
    # Bayu sudah check-in di event selesai
    ...
    # --- 7. BUAT SOAL DEMO ---
    # Soal untuk Event 1 (Workshop Advanced Funding)
