...
app.config['REMEMBER_COOKIE_DURATION'] = timedelta(days=3650) # 10 Tahun
...
app.config['MAIL_PASSWORD'] = os.getenv("MAIL_PASSWORD")# 16-digit App Password
...
def save_event_questions(event_id, form_data):
    ...
    # Hapus soal lama (cara paling aman untuk update bulk)
    EventQuestion.query.filter_by(event_id=event_id).delete()
    
    for i in range(1, 6):
        q_text = form_data.get(f'q_{i}_text')
        # Hanya simpan jika teks pertanyaan diisi
        if q_text:
...
# --- PERINTAH UNTUK DIJALANKAN OLEH SCHEDULER ---
    ...
    for event in events_tomorrow:
        ...
        for ticket in tickets:
            ...
            if user and user.fcm_token:
                ...
                # --- Konversi waktu ke WIB untuk ditampilkan di notifikasi ---
...
class BusinessProfile(db.Model):
    ...
    # Relasi ke sub-tabel (multi-entry)
...
def send_verification_email(user_email, otp_code):
    ...
    <div style="font-family: Arial, sans-serif; line-height: 1.6;">
        ...
        <p style="font-size: 32px; font-weight: bold; letter-spacing: 5px; margin: 20px 0; padding: 10px; background-color: #f4f4f4; width: 200px; text-align: center;">
            {otp_code}
        </p>
    ...
    try:
        msg = Message(
            ...
            html=html_content, 
            body=f"Kode verifikasi Anda adalah: {otp_code}" # Fallback
        )
        mail.send(msg)
...
def api_register():
    ...
    # Cek duplikasi (untuk user yang sudah terverifikasi)
    ...
    # Hapus user lama yang belum terverifikasi dengan email/hp yang sama
    ...
    # Buat user baru (tapi belum diverifikasi)
    while True:
...
def api_login():
    ...
    if user and user.check_password(password):
        if not user.is_verified:
            # Jika user belum verifikasi, kirim ulang OTP
            otp = generate_otp()
            user.verification_otp = otp
            user.otp_expiry = datetime.utcnow() + timedelta(minutes=10)
            user.last_otp_sent = datetime.utcnow()
            db.session.commit()
            send_verification_email(user.email, otp) # Kirim ulang
...
def verify_email():
    ...
    if user.verification_otp == otp:
        ...
        user.verification_otp = None # Hapus OTP setelah dipakai
...
def verify_otp_and_reset_password():
    ...
    if user.verification_otp == otp:
        ...
        user.otp_expiry = None
        user.is_verified = True # Pastikan terverifikasi
        db.session.commit()
...
def resend_otp():
    ...
    # Jika lolos cooldown, kirim ulang
    otp = generate_otp()
    user.verification_otp = otp
    user.otp_expiry = datetime.utcnow() + timedelta(minutes=10)
    user.last_otp_sent = datetime.utcnow() # Catat waktu kirim terakhir
...
def change_verification_email():
    ...
    # 1. Cek apakah email baru sudah dipakai akun terverifikasi
    ...
    # 2. Cari user unverified dengan email lama
    ...
    # 3. Hapus akun unverified lain yang mungkin pakai email baru
    User.query.filter_by(email=new_email, is_verified=False).delete()
    db.session.commit()

    # 4. Update email, generate OTP baru, kirim
    otp = generate_otp()
    user.email = new_email # <-- Update emailnya
    user.verification_otp = otp
    user.otp_expiry = datetime.utcnow() + timedelta(minutes=10)
    user.last_otp_sent = datetime.utcnow() # Reset cooldown
    
    if send_verification_email(user.email, otp):
        db.session.commit()
        return jsonify({'message': f'OTP telah dikirim ke email baru: {new_email}', 'new_email': new_email}), 200
    else:
        db.session.rollback() # Batalkan perubahan email jika kirim gagal
...
def get_events():
    ...
    visible_events = Event.query.filter(
        Event.tgl_buka_pendaftaran <= now_utc,    # 1. Pendaftaran sudah buka
        Event.tgl_tutup_pendaftaran >= now_utc,   # 1. Pendaftaran belum tutup
        Event.tgl_selesai_event > now_utc,      # 2. Event belum selesai
        Event.is_archived == False              # 3. Event tidak di-arsip
    ).order_by(Event.tgl_mulai_event.asc()).all() # Diurutkan (ASC)
...
def buy_ticket():
    ...
    # Validasi #1: Cek duplikasi tiket
    ...
    # --- VALIDASI #5: Cek Syarat Data UMKM ---
    if event.is_umkm_data_required:
        # Cek apakah user punya minimal 1 profile bisnis
...
def get_user_tickets(user_id):
    ...
    results = []
    for t in tickets:
        status = 'READY' # Default: Siap Check-in (QR / Join Online)
        label = 'Tersedia' # Teks untuk di Chip aplikasi
        
        if t.is_checked_in:
            if not t.event.has_pre_post_test:
                # Skenario A: Sudah Check-in, Tidak ada tes
                status = 'DONE'
                label = 'Selesai'
            else:
                # Skenario B: Ada Tes, Cek progress user
        ...
        # Override label jika belum check-in tapi Online
        ...
        results.append({
            ...
            'status': status, # <-- Field Baru
            'status_label': label # <-- Field Baru
        })
        
    return jsonify(results)

# 1. Cek Status Test User
...
def submit_test_api(event_id):
    data = request.json # Format: {'answers': { '1': 'A', '2': 'B' ... } }
...
def get_ticket_status(ticket_code):
    ...
    # Keamanan: Pastikan hanya pemilik tiket yang bisa bertanya
    if ticket.user_id != current_user.id:
        return jsonify(message="Akses ditolak"), 403
        
    # Kirim status check-in tiket
    return jsonify({
...
def update_user_profile(user_id):
    ...
    if 'has_business' in data: # Hanya update jika dikirim
...
# Endpoint Serve Upload 
@app.route('/uploads/<path:filename>')
def serve_upload(filename):
    return send_from_directory(app.config['UPLOAD_FOLDER'], filename)


# --- API UNTUK CRUD DATA UMKM ---
# Fase 1: API Menerima 1 set data per sub-tabel

# GET List semua UMKM milik user
...
# GET Detail 1 UMKM (untuk form edit)
...
def get_business_detail(id):
    ...
    # Ambil 1 data dari sub-tabel (jika ada)
    ...
    return jsonify({
        ...
        # Data dari sub-tabel (Fase 1: 1 entry)
...
# Helper untuk 'upsert' (update-or-insert) data sub-tabel Fase 1
...
# POST untuk Buat/Edit UMKM (Satu fungsi untuk create dan edit)
@app.route('/api/user/businesses/submit', methods=['POST'])
@login_required
def submit_user_business():
    data = request.json
    business_id = data.get('business_id') # Akan None jika 'Create'
    ...
    try:
        # --- Update Tabel Utama (BusinessProfile) ---
        ...
        # Commit dulu untuk mendapatkan business.id (jika CREATE mode)
        if not business_id:
            db.session.commit() 
        
        # --- Update Sub-Tabel (Fase 1: 1 entry per sub-tabel) ---
...
def delete_user_business(id):
    business = BusinessProfile.query.filter_by(id=id, user_id=current_user.id).first_or_404()
    # (Opsional) Hapus file logo dari server
...
def get_event_join_url(event_id):
    ...
    # Keamanan: Pastikan event-nya online DAN user punya tiket
    if event.tempat_event != 'Online':
    ...
    # Jika lolos, kirim link-nya
    return jsonify({
        'join_url': event.online_event_url
    })

# ======================================================
# --- ROUTE UNTUK DASBOR WEB (ADMIN & PANITIA) ---
# ======================================================
...
def new_event():
    if request.method == 'POST':
        ...
        def convert_to_utc_naive(form_time_string):
            ...
            # 1. Parse string dari form (ini adalah WIB Naive)
            dt_naive_wib = datetime.strptime(form_time_string, '%Y-%m-%dT%H:%M')
            # 2. Beri tahu Python ini adalah 'Asia/Jakarta'
            dt_aware_wib = LOCAL_TZ.localize(dt_naive_wib)
            # 3. Konversi ke UTC, lalu hapus info timezone agar jadi Naive
        ...
        if notif_title: 
            ...
            # Jika admin mengosongkan body, kita isi otomatis
            ...
            send_broadcast_notification(
                title=notif_title,  # <-- Judul kustom dari admin
                body=body_to_send,    # <-- Body kustom (atau fallback)
...
def edit_event(event_id):
    ...
    def convert_to_wib_string(utc_naive_dt):
        ...
        # 1. Beri tahu Python ini adalah UTC
        dt_aware_utc = pytz.utc.localize(utc_naive_dt)
        # 2. Konversi ke 'Asia/Jakarta'
        dt_aware_wib = dt_aware_utc.astimezone(LOCAL_TZ)
        # 3. Format ke string yang diterima input datetime-local
...
def reports():
    ...
    # Grafik akan mengikuti sorting tabel.
    chart_labels = [data['event'].title for data in sorted_data]
    
    # Tentukan data apa yang akan ditampilkan di grafik
    # Jika user sort by 'date', kita tampilkan 'registered' sebagai default
    if sort_by == 'checked_in':
        chart_values = [data['checked_in'] for data in sorted_data]
        chart_title = 'Peserta Hadir'
    elif sort_by == 'rate':
        chart_values = [round(data['rate'], 1) for data in sorted_data] # Bulatkan
        chart_title = 'Tingkat Kehadiran (%)'
    else: # Default (termasuk 'date' dan 'registered')
        chart_values = [data['registered'] for data in sorted_data]
        chart_title = 'Peserta Terdaftar'
    # ----------------------------------------
    
    # --- 6. Siapkan Data untuk Dropdown ---
    months = [
    ...
    return render_template(
        ...
        # --- Kirim data chart ke template ---
        chart_labels=chart_labels,
        chart_values=chart_values,
        chart_title=chart_title
    )

# Helper function untuk mengubah data UMKM jadi dictionary LENGKAP
def serialize_business(business):
    if not business:
        return None
    
    # Ambil 1 data dari sub-tabel (sesuai logic Fase 1 kamu)
    marketplace = business.marketplaces.first()
    license = business.licenses.first()
    finance = business.finances.first()
    npwp = business.npwps.first()
    ...
    return {
        # --- 1. Info Dasar (dari BusinessProfile) ---
        'business_name': business.business_name,
        'business_type': business.business_type,
        
        # --- 2. Alamat (dari BusinessProfile) ---
        'address_same_as_home': business.address_same_as_home,
        'address_province': business.address_province,
        'address_city': business.address_city,
        'address_district': business.address_district,
        ...
        # --- 3. Legalitas & Keuangan (dari BusinessProfile) ---
        'has_license': business.has_license,
        'legal_entity': business.legal_entity,
        'has_npwp': business.has_npwp,
        'financial_report_type': business.financial_report_type,
        ...
        # --- 4. Kontak & Operasional (dari BusinessProfile) ---
        'business_phone': business.business_phone,
        'business_email': business.business_email,
        'operating_since': business.operating_since,

        # --- 5. Data dari Sub-tabel ---
        'marketplace_type': marketplace.marketplace_type if marketplace else None,
        'url': marketplace.url if marketplace else None,
...
@login_required
@role_required(['admin']) # Hanya admin yang boleh
...
def download_csv(event_id):
    ...
    si = io.StringIO()
    writer = csv.writer(si)
    writer.writerow(selected_columns) # Tulis Header
